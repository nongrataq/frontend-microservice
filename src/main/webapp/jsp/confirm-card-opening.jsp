...
<html>
<head>
    <title>Подтвердить открытие карты</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<main>
    <div class="container">
        <h2>Подтвердить открытие карты</h2>
        <div class="confirm-block">
            <p>Вы собираетесь открыть новую карту:</p>
            <p><strong><c:out value="${cardName}"/></strong></p>
            <p>Это действие завершит процесс открытия. Карта будет доступна в вашем профиле.</p>
        </div>

        <div class="actions">
            <form action="${pageContext.request.contextPath}/create-card" method="post">
                <input type="hidden" name="cardProductId" value="${cardProductId}">
                <button type="submit" class="grey-btn main">Открыть карту</button>
            </form>
            <a href="${pageContext.request.contextPath}/profile" class="grey-btn secondary">Отмена</a>
        </div>
    </div>
</main>
<footer>
